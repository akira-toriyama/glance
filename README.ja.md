...
<!-- スクリーンショット
docs/img/ に panel の screenshot を置いて、ここで参照する想定。

![markdown demo](docs/img/markdown-demo.png)
![hud toast](docs/img/hud-toast.png)

撮り方: ./bin/glance --markdown ... を起動 → Cmd+Shift+4 + Space + panel をクリック。
-->
...
### `swift test` は full Xcode が必須

ローカルで `swift test` が `no such module 'XCTest'` で落ちる場合は
Command Line Tools のみ入っている状態。XCTest framework は Xcode 本体
(App Store の Xcode.app) に同梱されています。一度きりの `xcode-select`
切替で解決 →
[CONTRIBUTING.md → Tests need full Xcode](CONTRIBUTING.md#tests-need-full-xcode)
参照。CI (macos runner) は Xcode が入っているので PR は常に full test
される。
