...
| **macOS 26+ only** | sill's `Palette` / `PaletteKit` carry a macOS 26 floor (t-tbar), and the family targets the latest macOS only. `.nonactivatingPanel` + the swift-markdown / Highlightr renderer (`MarkdownRenderer`) lineage sit well inside that. |
